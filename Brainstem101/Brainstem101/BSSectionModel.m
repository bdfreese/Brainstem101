...
- (id)init
{
    ...
    if (self) {
        ...
        nuclei = [NSNumber numberWithInt:0];
        tracts = [NSNumber numberWithInt:0];
        perfusions = [NSNumber numberWithInt:0];
...
-(void) addData{
    ...
    [allSections addObject:nn3]; //CONFUSED
    ...
    [allSections addObject:nn8]; // CONFUSED -- two 4's and nuc and tract?
    ...
    [allSections addObject:n10]; //THIS IS A TRACT AND A NUCLEIUS
    ...
    [allSections addObject:t3]; // CONFUSED, fibers/tract, and no section 6
    ...
    [allSections addObject:tt1]; // CONFUSED -- streaks on 2 and 3, is this right?
    ...
    [allSections addObject:t6]; // CONFUSED
    ...
    [allSections addObject:tt2]; // CONFUSED -- peduncle == tract? medial vs. middle??
...
-(int)numberOfType:(int)type{
    ...
    for (int i = 0 ; i < [allSections count]; i++) {
        if ([[allSections objectAtIndex:i] sectionType] == [NSNumber numberWithInt:type]) {
            counter++;
        }
    }
    return counter;
